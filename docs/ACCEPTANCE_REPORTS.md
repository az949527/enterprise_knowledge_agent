...
### LLM 兜底（2026-08-07 补充）

- 新增 `app/lite/computation_llm.py`：`resolve_column`（白名单内选列，输出校验）、
  `synthesize_mixed`（把精确数字 + 检索片段喂 LLM 写综合回答）、
  `arun_computation_with_fallback`（确定性澄清时用 LLM 消歧重算）。
- `desktop_query` 计算分支在 `use_llm + api_key` 时走兜底；无 key/离线自动回退纯规则 + 澄清。
- 安全红线不变：LLM 只填参数/写综合段落，不写 SQL、不碰数据；选列结果必须命中白名单。

### 边界与后续

- 执行层从"映射到受控 SQLite"改为纯 Python 白名单计算（零注入面，语义等价）。
- 弱算子（最高/最低）在混合索引下需显式表范围，避免与文档措辞混淆。
- 跨文件列名不一致由 LLM 兜底选列；混合文档由 LLM 综合回答兜底（均有 key 时生效）。
