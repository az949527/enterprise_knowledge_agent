    ...
    *,
    ...
    if not sources:
        return {
            ...
            "mode": "empty",
            "context": context,
            "llm": {"enabled": bool(resolved_api_key), "usage": None},
        }

    if use_llm:
        result = await _answer_with_llm(query, context, resolved_api_key, resolved_base_url, resolved_model)
        if result["answer"]:
        ...
        llm_metadata = {"enabled": bool(resolved_api_key), "usage": None}
